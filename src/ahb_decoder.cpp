...
DEVICE_ID AHBDecoder::AddrDecode(sc_uint<BW> addr) {
    ...
    if (DMA_ADDR_START <= addr && addr <= DMA_ADDR_END) {
        sel = SLAVE_DMA_ID;
        LOG_INFO(logger, "addr[{:#010x}], slave dma be selected, HSELx={:#b}.", addr.to_uint(), sel.to_uint());
    } else if (RAM_ADDR_START <= addr && addr <= RAM_ADDR_END) {
        sel = SLAVE_RAM_ID;
        LOG_INFO(logger, "addr[{:#010x}], slave ram be selected, HSELx={:#b}.", addr.to_uint(), sel.to_uint());
    } else {
