...
class AHBAribter : public sc_module {
  ...
  public:
    // 输入信号
    sc_in<sc_uint<MASTER_CNT>> HBUSREQx; // 每个比特表示一个主设备的总线请求信号,允许多个bit位为1
    sc_in<sc_uint<MASTER_CNT>> HLOCKx; // 每个比特表示一个主设备的总线锁信号,允许多个bit位为1
    ...
    // 输出信号
    sc_out<sc_uint<MASTER_CNT>> HGRANTx; // 每个bit位表示一个主设备的总线控制权,只允许一个bit位为1
